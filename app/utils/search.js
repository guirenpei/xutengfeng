...
function analyseSearch(html) {
  ...
  if (html === undefined) {
    ...
    const lists = $('.result-list .result-item');
    lists.map(function(i, el) {
      ...
      let number = parseInt(link.substring(20, 21), 10) || 0;
...
function *searchResults(keyword) {
  const search = querystring.escape(keyword);
  const results = [];
  try {
    ...
    const res5 = yield request({
      url: `http://so.kanshu.com/search/searchlist.php?&keys=${search}&sort=1`,
      timeout: 3000,
    });
    yield isNull(results, yield getResult(analyseSearch(res1), keyword));
    yield isNull(results, yield getResult(analyseSearch(res2), keyword));
    yield isNull(results, yield getResult(analyseSearch(res3), keyword));
    yield isNull(results, yield getResult(analyseSearch(res4), keyword));
    yield isNull(results, yield getResult(analyseSearchKanShu(res5), keyword));
  } catch (error) {
    console.log(error);
  }
  return results;
...
function getResult(datas, keyword) {
  return new Promise((resolve, reject) => {
    let result = datas[0];
    for (const data of datas) {
      if (data.storyname === keyword) {
        result = data;
      }
    }
    resolve(result);
